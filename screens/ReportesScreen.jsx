...
const ReportesScreen = ({ navigation }) => {
  ...
  const obtenerDiaSemana = (fecha) => {
    ...
    return dias[fechaConvertida.getDay()];
  };
  ...
  const reportes = useMemo(() => {
    ...
    entregas.forEach((entrega) => {
      ...
      if (
        entrega.fecha === fechaActual
      ) {
        return;
      }

      if (!agrupados[entrega.fecha]) {
        agrupados[entrega.fecha] = [];
      }

      agrupados[entrega.fecha].push(
        entrega
      );
    });

    return Object.entries(agrupados)
      .map(([fecha, entregasDia]) => {
        const totalVendido = entregasDia.reduce(
          (total, entrega) =>
            total + Number(entrega.total || 0),
          0
        );

        const totalRecibido = entregasDia.reduce(
          (total, entrega) =>
            total + Number(entrega.abona || 0),
          0
        );

        /*
          IMPORTANTE:
          No usamos totalVendido - totalRecibido
          porque cada entrega ya posee su saldoPendiente.

          Esto también conserva correctamente
          los saldos negativos.
        */
        const saldoPendiente = entregasDia.reduce(
          (total, entrega) =>
            total +
            Number(entrega.saldoPendiente || 0),
          0
        );
        ...
        const productosVendidos = entregasDia.reduce(
          (total, entrega) => {
  ...
  const renderReporte = ({ item }) => (
    ...
    >
      ...
      <View style={styles.resumenDinero}>
        <View style={styles.columnaDinero}>
          <Text style={styles.labelPequeno}>
            Total vendido
          </Text>

          <Text style={styles.valorVerde}>
            {dinero(item.totalVendido)}
          </Text>
        </View>
  ...
  Ventas y entregas registradas
      ...
      <View style={styles.infoBox}>
        ...
        <Text style={styles.infoTexto}>
          ...
          con base en las entregas registradas cada día.
        </Text>
      </View>
      ...
      {reportes.length > 0 && (
        <View style={styles.tituloHistorial}>
          <Ionicons
        ...
        ListEmptyComponent={
          <View style={styles.vacio}>
            ...
            <Text style={styles.vacioTexto}>
              Los reportes aparecerán automáticamente
              cuando se registren entregas.
            </Text>
          </View>
        }
      />
